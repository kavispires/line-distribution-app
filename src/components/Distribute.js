...
class Distribute extends Component {
  ...
  componentWillMount () {
    const currentBand = this.props.app.artists[this.props.app.currentBand];
    console.log(currentBand);
    if (currentBand !== undefined && currentBand.members.length !== this.props.distribute.durations.length) {
      const newArray = new Array(currentBand.members.length).fill(0);
  ...
  render () {
    ...
    const currentBand = this.props.app.artists[this.props.app.currentBand];
    const boxSize = currentBand ? boxSizeClass(currentBand.members.length) : 'small';
