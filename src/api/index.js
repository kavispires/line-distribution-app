    ...
    switch (route.root) {
      ...
      case 'artists':
        ...
        if (route.referenceId && route.subPath === 'units') {
          result = await this.getFunctions().fetchArtistUnits(
            route.referenceId
          );
        }
        // API/artists/<id>
        else if (route.referenceId) {
          result = await this.getFunctions().fetchArtist(route.referenceId);
        }
        // API/aritsts
        else {
          result = await this.getFunctions().fetchArtists();
        }
        break;
      // API/colors
      case 'colors':
        result = await this.getFunctions().fetchColors();
        break;
      case 'members':
        // API/members/<id>
        if (route.referenceId) {
          result = await this.getFunctions().fetchMember(route.referenceId);
        }
        // API/members
        else {
      ...
      case 'songs':
        // API/songs/<id>
        if (route.referenceId) {
          result = await this.getFunctions().fetchSong(route.referenceId);
        }
        // API/songs
        else {
          result = await this.getFunctions().fetchSongs();
        }
        break;
      // API/units
      case 'units':
        ...
        if (route.referenceId && route.subPath === 'distributions') {
          result = await this.getFunctions().fetchUnitDistributions(
            route.referenceId
          );
        }
        // API/units/<id>/members
        else if (route.referenceId && route.subPath === 'units') {
          result = await this.getFunctions().fetchUnitMembers(
            route.referenceId
          );
        }
        // API/units/<id>
        else if (route.referenceId) {
          result = await this.getFunctions().fetchUnit(route.referenceId);
        }
        break;
      // API/users
      case 'users':
        // API/users/<id>
        result = await this.getFunctions().fetchUser(route.referenceId);
        break;
      default:
  ...
  getFunctions() {
    return {
      ...
      fetchArtistUnits: async id => {
        const artist = await this.getFunctions().fetchArtist(id);
        const response = await artist.attributes.units.map(unitId =>
          this.getFunctions().fetchUnit(unitId)
        );
      ...
      fetchUnitDistributions: async id => {
        const unit = await this.getFunctions().fetchUnit(id);
        const response = await unit.attributes.distributions.map(
          distributionId =>
            this.getFunctions().fetchDistribution(distributionId) // TO-DO: this function
        );
        return Promise.all(response);
      },
      // Fetches units from a single artist
      fetchUnitMembers: async id => {
        const unit = await this.getFunctions().fetchUnit(id);
        const response = await unit.attributes.members.map(memberId =>
          this.getFunctions().fetchMember(memberId)
        );
        return Promise.all(response);
      },
      // Fetches single user
